...
class ChangeList(object):
    def __init__(self,config,queryset):
        ...
        page_obj = Pagination(current_page, total_count, self.request.path_info, self.request.GET, per_page_count=3)
...
class StarkConfig(object):
    ...
    def delete(self,obj=None,is_header=False):
        ...
        if query_str:
            ...
            params = QueryDict(mutable=True)
            params[self._query_param_key] = query_str
            return mark_safe('<a href="%s?%s">删除</a>' % (self.get_delete_url(obj.id), params.urlencode(),))  # /stark/app01/userinfo/
        return mark_safe('<a href="%s">删除</a>' % (self.get_delete_url(obj.id),))  # /stark/app01/userinfo/
